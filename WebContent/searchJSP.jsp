...
<body>
	...
	<% 
		...
		Product result = dao.getProduct(Integer.parseInt(request.getParameter("searchId")));
		if(result != null){
	%>
						<thead>
						...
						<tbody>
								...
								<tr>
									<td class="column1"><%= result.getProductId()%></td>
									<td class="column2"><%= result.getProductName()%></td>
									<td class="column3"><%= result.getProductCategory()%></td>
									<td class="column6"><%= result.getProductPrice()%></td>
								</tr>
									
						</tbody>
	<%
	} else {
		response.sendRedirect("productNotFound.html");
	}
	%>
